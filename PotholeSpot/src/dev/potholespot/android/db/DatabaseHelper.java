...
public class DatabaseHelper extends SQLiteOpenHelper
{
   ...
   /*
    * public SQLiteDatabase openDB(SQLiteDatabase db) { db = this.getWritableDatabase(); return db; }
    */
   ...
   @Override
   public void onUpgrade(final SQLiteDatabase db, int current, final int targetVersion)
   {
      ...
      {
         for (final String statement : Waypoints.UPGRADE_STATEMENT_7_TO_8)
         {
      ...
      new Thread()
         {
            ...
            public void run()
            {
               final SQLiteDatabase sqldb = getWritableDatabase();
   ...
   int bulkInsertWaypoint(final long trackId, final long segmentId, final ContentValues[] valuesArray)
   {
      ...
      try
      {
         for (final ContentValues args : valuesArray)
         {
            args.put(WaypointsColumns.SEGMENT, segmentId);

            final long id = sqldb.insert(Waypoints.TABLE, null, args);
            if (id >= 0)
            {
               inserted++;
            }
   ...
   int bulkInsertLocations(final long labelId, final ContentValues[] valuesArray)
   {
      if (labelId < 0)
      {
      ...
      try
      {
         for (final ContentValues args : valuesArray)
         {
            ...
            final long id = sqldb.insert(Locations.TABLE, null, args);
            if (id >= 0)
            {
               inserted++;
            }
    ...
    * @return
    */
   long insertWaypoint(final long trackId, final long segmentId, final Location location)
   {
      if (trackId < 0 || segmentId < 0)
      {
         throw new IllegalArgumentException("Track and segments may not the less then 0.");
      ...
      final long waypointId = sqldb.insert(Waypoints.TABLE, null, args);

      final ContentResolver resolver = this.mContext.getContentResolver();
      final Uri notifyUri = Uri.withAppendedPath(Tracks.CONTENT_URI, trackId + "/segments/" + segmentId + "/waypoints");
   ...
   public long insertLocation(final Location location)
   {
      final SQLiteDatabase sqldb = getWritableDatabase();
      ...
      final long locationId = sqldb.insert(Locations.TABLE, null, args);
      sqldb.close();
   ...
   public void insertLabel(final String label, final Long labelTime)
   {
      ...
      {
         final ContentValues args = new ContentValues();
         args.put(LabelsColumns.CREATION_TIME, labelTime);
         args.put(LabelsColumns.NAME, label);
   ...
   public long insert_xyz(final float x, final float y, final float z, final Location location)
   {

      final SQLiteDatabase sqldb = getWritableDatabase();

      final ContentValues args = new ContentValues();
      args.put(XYZColumns.TIME, location.getTime());
      args.put(XYZColumns.X, x);
      args.put(XYZColumns.Y, y);
      args.put(XYZColumns.Z, z);
      final long xyzId = sqldb.insert(Xyz.TABLE, null, args);
      sqldb.close();
      Log.d(TAG, "acceleration value stored ");
      return xyzId;
   }

   public void delete_xyz(final long id)
   {
      final SQLiteDatabase db = getWritableDatabase();
      if (db == null)
      {
    ...
    */
   public ContentValues get(final long id)
   {
      ...
      final Cursor cur = db.rawQuery("select name, creationtime from labels where _id = ?", new String[] { String.valueOf(id) });
      if (cur.moveToNext())
      {
   ...
   public Cursor selectLabelsRecords()
   {
      ...
      final String[] cols = new String[] { BaseColumns._ID, LabelsColumns.NAME };
      final Cursor mCursor = database.query(true, Labels.TABLE, cols, null, null, null, null, null, null);
      if (mCursor != null)
      {
         mCursor.moveToFirst();
      }
    ...
    * @return
    */
   long insertMedia(final long trackId, final long segmentId, final long waypointId, final String mediaUri)
   {
      ...
      final long mediaId = sqldb.insert(Media.TABLE, null, args);

      final ContentResolver resolver = this.mContext.getContentResolver();
      final Uri notifyUri = Uri.withAppendedPath(Tracks.CONTENT_URI, trackId + "/segments/" + segmentId + "/waypoints/" + waypointId + "/media");
      resolver.notifyChange(notifyUri, null);
    ...
    * @return
    */
   long insertOrUpdateMetaData(final long trackId, final long segmentId, final long waypointId, final String key, final String value)
   {
      long metaDataId = -1;
      ...
      final String whereClause = MetaDataColumns.TRACK + " = ? AND " + MetaDataColumns.SEGMENT + " = ? AND " + MetaDataColumns.WAYPOINT + " = ? AND " + MetaDataColumns.KEY + " = ?";
      final String[] whereArgs = new String[] { Long.toString(trackId), Long.toString(segmentId), Long.toString(waypointId), key };

      final SQLiteDatabase sqldb = getWritableDatabase();
      final int updated = sqldb.update(MetaData.TABLE, args, whereClause, whereArgs);
      if (updated == 0)
      {
      ...
      else
      {
         ...
         try
         {
            c = sqldb.query(MetaData.TABLE, new String[] { BaseColumns._ID }, whereClause, whereArgs, null, null, null);
            if (c.moveToFirst())
            {
   ...
   int deleteTrack(final long trackId)
   {
      final SQLiteDatabase sqldb = getWritableDatabase();
      int affected = 0;
      Cursor cursor = null;
      ...
      try
      {
         ...
         cursor = sqldb.query(Segments.TABLE, new String[] { BaseColumns._ID }, SegmentsColumns.TRACK + "= ?", new String[] { String.valueOf(trackId) }, null, null, null, null);
         ...
         // Delete the track
         affected += sqldb.delete(Tracks.TABLE, BaseColumns._ID + "= ?", new String[] { String.valueOf(trackId) });
         // Delete remaining meta-data
         affected += sqldb.delete(MetaData.TABLE, MetaDataColumns.TRACK + "= ?", new String[] { String.valueOf(trackId) });

         cursor = sqldb.query(MetaData.TABLE, new String[] { BaseColumns._ID }, MetaDataColumns.TRACK + "= ?", new String[] { String.valueOf(trackId) }, null, null, null, null);
         if (cursor.moveToFirst())
         {
            do
            {
    ...
    * @return
    */
   int deleteMedia(final long mediaId)
   {
      final SQLiteDatabase sqldb = getWritableDatabase();
      ...
      try
      {
         cursor = sqldb.query(Media.TABLE, new String[] { MediaColumns.TRACK, MediaColumns.SEGMENT, MediaColumns.WAYPOINT }, BaseColumns._ID + "= ?", new String[] { String.valueOf(mediaId) }, null,
               null, null, null);
         if (cursor.moveToFirst())
         {
      ...
      final int affected = sqldb.delete(Media.TABLE, BaseColumns._ID + "= ?", new String[] { String.valueOf(mediaId) });
   ...
   int deleteMetaData(final long metadataId)
   {
      final SQLiteDatabase sqldb = getWritableDatabase();
      ...
      try
      {
         cursor = sqldb.query(MetaData.TABLE, new String[] { MetaDataColumns.TRACK, MetaDataColumns.SEGMENT, MetaDataColumns.WAYPOINT }, BaseColumns._ID + "= ?",
      ...
      final int affected = sqldb.delete(MetaData.TABLE, BaseColumns._ID + "= ?", new String[] { String.valueOf(metadataId) });
    ...
    * @return
    */
   int deleteSegment(final SQLiteDatabase sqldb, final long trackId, final long segmentId)
   {
      int affected = sqldb.delete(Segments.TABLE, BaseColumns._ID + "= ?", new String[] { String.valueOf(segmentId) });

      // Delete all waypoints from segments
      affected += sqldb.delete(Waypoints.TABLE, WaypointsColumns.SEGMENT + "= ?", new String[] { String.valueOf(segmentId) });
      // Delete all media from segment
      affected += sqldb.delete(Media.TABLE, MediaColumns.TRACK + "= ? AND " + MediaColumns.SEGMENT + "= ?", new String[] { String.valueOf(trackId), String.valueOf(segmentId) });
      // Delete meta-data
      affected += sqldb.delete(MetaData.TABLE, MetaDataColumns.TRACK + "= ? AND " + MetaDataColumns.SEGMENT + "= ?", new String[] { String.valueOf(trackId), String.valueOf(segmentId) });
   ...
   int updateTrack(final long trackId, final String name)
   {
      int updates;
      final String whereclause = BaseColumns._ID + " = " + trackId;
      final ContentValues args = new ContentValues();
      ...
      final ContentResolver resolver = this.mContext.getContentResolver();
      final Uri notifyUri = ContentUris.withAppendedId(Tracks.CONTENT_URI, trackId);
      resolver.notifyChange(notifyUri, null);

      return updates;
   }

   int updateLabel(final long labelId, final String name)
   {
      int updates;
      final String whereclause = BaseColumns._ID + " = " + labelId;
      final ContentValues args = new ContentValues();
      ...
      final ContentResolver resolver = this.mContext.getContentResolver();
      final Uri notifyUri = ContentUris.withAppendedId(Labels.CONTENT_URI, labelId);
      resolver.notifyChange(notifyUri, null);
    ...
    * @return
    */
   int updateMetaData(final long trackId, final long segmentId, final long waypointId, final long metadataId, final String selection, final String[] selectionArgs, final String value)
   {
      {
         ...
         {
            whereclause = MetaDataColumns.TRACK + " = ? AND " + MetaDataColumns.SEGMENT + " = ? AND " + MetaDataColumns.WAYPOINT + " = ? AND " + MetaDataColumns.KEY + " = ? ";
            whereParams = new String[] { Long.toString(trackId), Long.toString(segmentId), Long.toString(waypointId), selectionArgs[0] };
         }
         final ContentValues args = new ContentValues();
         args.put(MetaDataColumns.VALUE, value);

         final int updates = sqldb.update(MetaData.TABLE, args, whereclause, whereParams);
    ...
    * @return
    */
   long toNextTrack(final String name)
   {
      ...
      final SQLiteDatabase sqldb = getWritableDatabase();
      final long trackId = sqldb.insert(Tracks.TABLE, null, args);
   ...
   long toNextLabel(final String name)
   {
      final long currentTime = new Date().getTime();
      final ContentValues args = new ContentValues();
      args.put(LabelsColumns.NAME, name);
      ...
      final SQLiteDatabase sqldb = getWritableDatabase();
      final long labelId = sqldb.insert(Labels.TABLE, null, args);
    ...
    * @return
    */
   long toNextSegment(final long trackId)
   {
      final SQLiteDatabase sqldb = getWritableDatabase();

      final ContentValues args = new ContentValues();
      args.put(SegmentsColumns.TRACK, trackId);
      final long segmentId = sqldb.insert(Segments.TABLE, null, args);
   ...
   public ArrayList<Cursor> getData(final String Query)
   {
      //get writable database
      final SQLiteDatabase sqlDB = getWritableDatabase();
      final String[] columns = new String[] { "mesage" };
      //an array list of cursor to save two cursors one has results from the query 
      //other cursor stores error message if any errors are triggered
      final ArrayList<Cursor> alc = new ArrayList<Cursor>(2);
      final MatrixCursor Cursor2 = new MatrixCursor(columns);
      alc.add(null);
      alc.add(null);

      try
      {
         final String maxQuery = Query;
         //execute the query results will be save in Cursor c
         final Cursor c = sqlDB.rawQuery(maxQuery, null);
